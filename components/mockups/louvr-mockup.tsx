...
type Fila = { ad: string; cpa: string; ctr: string; frec: string; badge: string; tono: string; pct: number }

const FILAS: Fila[] = [
  { ad: 'Retargeting · v3', cpa: '4,10 €', ctr: '2,8%', frec: '1,4', badge: 'Scale', tono: OK, pct: 88 },
  { ad: 'Lookalike 1% · UGC', cpa: '6,90 €', ctr: '1,9%', frec: '2,1', badge: 'Hold', tono: INK, pct: 55 },
  { ad: 'Broad · carrusel', cpa: '11,40 €', ctr: '0,9%', frec: '4,3', badge: 'Pause', tono: TERRA, pct: 22 },
  { ad: 'Interés · estático', cpa: '7,80 €', ctr: '1,3%', frec: '3,0', badge: 'Refresh', tono: 'rgba(21,20,18,0.5)', pct: 40 },
]

export function LouvrMockup() {
  return (
    <MockupFrame title="Motor de ranking" context="capa 1 reglas · capa 2 Claude" status="status: badge asignado" tone={OK}>
      ...
      <div className="border border-line bg-surface">
        <div className="grid grid-cols-[1.6fr_0.8fr_0.7fr_0.7fr_0.9fr] gap-2 border-b border-hair px-3 py-2 font-mono text-[8.5px] uppercase tracking-[0.12em] text-fg-faint">
          ...
          <span className="text-right">Frec</span>
        ...
        {FILAS.map((f, i) => (
          ...
          >
            ...
            <span className="text-right font-mono text-[11px] tabular-nums text-fg-muted">{f.frec}</span>
      ...
      {/* Capa 2 — Claude razona sobre el ranking ya hecho */}
      <div className="mt-3 border border-ink bg-ink p-3 text-bg">
        ...
        <p className="mt-2 text-[12px] leading-relaxed text-[rgba(240,238,233,0.82)]">
          El broad de carrusel se marca <span className="text-bg">Pause</span>: la frecuencia ya en 4,3 explica el
          CPA disparado — está saturando a la misma audiencia. Rota creativo antes de reactivar. El retargeting
          aguanta escala: CTR alto y frecuencia baja.
        </p>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div>
            <p className="font-mono text-[8.5px] uppercase tracking-[0.12em] text-[rgba(240,238,233,0.5)]">Ranking determinista</p>
            <div className="mt-1.5">
              <Meter pct={100} tone="#F0EEE9" delay={1400} />
            </div>
          </div>
          <div>
            <p className="font-mono text-[8.5px] uppercase tracking-[0.12em] text-[rgba(240,238,233,0.5)]">Reporte semanal · auto</p>
            <div className="mt-1.5">
